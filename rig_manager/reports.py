...
def decision_to_dict(decision: Decision) -> dict[str, Any]:
    return {
        ...
        "pool_name": None if decision.pool is None else decision.pool.name,
        ...
        "pool_hashrate": None if decision.pool is None else (decision.pool.hashrate or None),
        "pool_fee_pct": None if decision.pool is None else (decision.pool.fee_pct or None),
...
def write_markdown(path: Path, decision: Decision) -> None:
    ...
    lines = [
        ...
        f"- Pool: `{data['pool_name'] or 'unknown'}`",
        f"- Pool server: `{data['pool_server'] or 'unknown'}`",
        f"- Pool port: `{data['pool_port'] or 'unknown'}`",
        f"- Pool hashrate: `{data['pool_hashrate'] or 'unknown'}`",
        f"- Pool fee: `{data['pool_fee_pct'] or 'unknown'}`%",
        "",
        "## Command",
        "",
        "```powershell",
